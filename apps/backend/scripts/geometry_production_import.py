"""Production-shaped Geometry source/RAG import runner with fail-closed guards."""
...
def _audit_manifest(manifest: dict[str, object]) -> dict[str, object]:
    ...
    return summarize_audit(audit_rows(rows, expected_subject_code="geometry"))


def build_geometry_import_plan(*, manifest: dict[str, object], target_env: str, dry_run: bool = True) -> dict[str, object]:
...
def execute_geometry_import_plan(
    *,
    ...
    plan = build_geometry_import_plan(manifest=manifest, target_env=target_env, dry_run=dry_run)
    ...
    with Session(engine) as session:
        with session.begin():
            if topic_ids:
                existing_material_ids = list(
                    session.execute(select(materials_table.c.id).where(materials_table.c.topic_id.in_(topic_ids))).scalars()
                )
            ...
            for material in materials:
                session.execute(
                    materials_table.insert().values(
                        ...
                        source=str(material.get("source_url") or material.get("source") or "geometry_source"),
                        file_path=None,
                        status="published",
                        source_type="text",
                        ai_confidence=None,
        ...
        if topic_ids:
            ...
            chunk_count = session.scalar(select(func.count()).select_from(chunks_table).where(chunks_table.c.material_id.in_(imported_material_ids))) or 0
        else:
            material_count = 0
            chunk_count = 0
    engine.dispose()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run guarded Geometry source/RAG import plan")
