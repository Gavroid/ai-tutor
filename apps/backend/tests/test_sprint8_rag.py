...
Sprint 3.5.2: RAG подключён к app/ai/service.py::explain_topic.
Тесты реактивированы (skip снят). Тесты проверяют:
- get_or_compute_embedding (с хэш-фоллбэком, без реального API)
- chunk_hash / text_hash (детерминированные)
