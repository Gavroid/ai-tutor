...
async def check_answer(payload: CheckIn, current: user_models.User = Depends(get_current_user)):
    svc = get_ai_service()
...
async def generate(payload: GenerateIn, db: Session = Depends(get_db), current: user_models.User = Depends(get_current_user)):
    topic = db.get(subj_models.Topic, payload.topic_id)
    if topic is None:
        raise HTTPException(404, "Topic not found")
    svc = get_ai_service()
...
@router.post("/chat")
async def chat(payload: ChatIn, db: Session = Depends(get_db), current: user_models.User = Depends(get_current_user)):
    svc = get_ai_service()
