...
for i in $(seq 1 60); do
  if docker exec "$TEMP_CONTAINER" pg_isready -U tutor >/dev/null 2>&1; then
...
if gunzip -c "$TEST_RESTORE_DIR/$LATEST_DB" 2>/dev/null | \
  docker exec -i "$TEMP_CONTAINER" psql -U tutor -d "$TEST_DB_NAME" -v ON_ERROR_STOP=0 >/tmp/restore_drill.log 2>&1; then
  log "✓ restore SUCCEEDED"
else
  log "✗ restore FAILED (see /tmp/restore_drill.log)"
  tail -20 /tmp/restore_drill.log | log
  docker rm -f "$TEMP_CONTAINER" >/dev/null 2>&1
  exit 1
fi
