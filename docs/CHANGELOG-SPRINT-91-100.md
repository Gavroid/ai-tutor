...
| 3.39 | ~~(skipped)~~ ✅ `920cb59` | **passlib → bcrypt-direct migration (TDD)**: 12 новых тестов фиксируют behavior (cost=12, $2b$ prefix, bool return, round-trip для паролей и 2FA кодов). Реальный fix — passlib 1.7.4 + bcrypt 4.0.1 давал шумный deprecation warning + silent 72-byte truncate (vs bcrypt 4.x raise ValueError → 500). requirements.txt: удалён passlib, оставлен bcrypt. | (не нужен — security refactor, behavior identical) |
| 4.1 | `4f42095` | **Single-source recommendations (parents → frontend)**: backend единственный источник. apps/backend/app/parents/{schemas,service}.py: ChildOverview +recommendations: list[ParentRecommendation] +review_topics: list[ReviewTopic]. frontend apps/frontend/app/parents/page.tsx: удалена buildParentRecommendations (~25 строк, magic numbers 0.6, 5×2, 7). apps/frontend/lib/api.ts: типы расширены. 4 RED→GREEN теста. | `20260907T070453Z-5c95974` (вместе с 4.2) |
| 4.2 | `6e47839` | **Mastery thresholds (WEAK_MASTERY_THRESHOLD=0.60, REVIEW_TOPICS_LIMIT=5)**: новые константы в parents/service.py, inline `0.6` → константа, UI `60%` → `WEAK_MASTERY_THRESHOLD_PCT`. Новая ReviewTopic schema + get_review_topics() функция (top-5 по last_reviewed_at, NULLs first, ASC, tie-breaker по topic_id). 7 тестов. | `20260907T070453Z-5c95974` (вместе с 4.1) |
| 4.3 | `db583d6` | **AI классифицирует ошибки + context-aware hints + A/B metrics**: pedagogical-ai улучшения. | (требует deploy; в сессии не развёрнут) |
| 4.4 | `508cc44` | **Smoke-test user + smoke.sh fallback**: dedicated `smoke-test@431a.ru` (id=82, role=admin, is_active=true, 2FA off) — ИСКЛЮЧЁН из whitelist. pg_dump снапшот перед destructive INSERT (`db-pre-smoke-user-20260907-064638.sql.gz`, 4.2 МБ). `/opt/ai-tutor/deploy/release/.smoke.env` (chmod 600, НЕ в git). smoke.sh: новый 1-й уровень fallback читает из .smoke.env. .gitignore: явное исключение `.smoke.env`. verify: `[smoke] OK: smoke прошёл` (9/9 шагов). | (production deploy не требуется — credentials file на проде, smoke.sh change применится при следующем deploy) |
| 4.5+ | `df7161b` | **Single-source daily_focus (parent/dashboard)**: ChildDashboard +daily_focus: list[str] (3 строки: weekly, focusToday, helpSignal). _parent_daily_focus() функция с edge case (total_attempts==0 → []). frontend apps/frontend/app/parent/dashboard/[studentId]/page.tsx: удалены IIFE для weeklySummary/tomorrowPlan/helpSignal (~14 строк, magic numbers 0.6, dash.weak_topics[0]). Dashboard type: +daily_focus: string[]. 2 теста. | `20260907T073517Z-5c95974` (вместе с quality batch) |
| Quality batch | `3a64811` `bdec943` `957dd3f` `05c533a` | **RU-format + RAG fix + CI gates fix + CSP verify**: ruff format (4 файла), mypy fix в rag.py:165 (явный list() cast), CI gate fix `app/observability_otel` → `app/observability_otel.py` (mypy path). CSP verify-тесты (3): подтверждают работу через nginx add_header для HTML + FastAPI middleware для API. | `20260907T073517Z-5c95974` (вместе с 4.5+) |
...
| 3.45 | `b4a8f6c` | **monthly doc-archive cron** (T5 из аудита): `scripts/docs-monthly-archive.sh` + `docs-monthly-archive.cron.example`. Автоматизирует Sprint 3.41 (140 файлов git mv в docs/archive/YYYY-MM/). Dry-run support. Cron закомментирован — активация требует решения владельца. | (cron — без deploy) |
| **Git-flow fix** | `27d6f5e` | **Единая ветка-истина `main` + deploy guard**: master ff-merged в main (+427 коммитов), `design-audit-2026-08-20-fixes` ff-merged (+ Sprint 3.39..4.5+ + revert RAG). Удалены 5 локальных + 3 remote feature-ветки (все коммиты сохранены в истории main). Guard в `deploy/release/deploy.sh`: exit 1 если ветка ≠ main (override `ALLOW_NON_MAIN_DEPLOY=1`). Guard в `.github/workflows/deploy.yml`: новый input `allow_non_main` (default false) + guard-шаг после checkout. Защита от повторения "main отстаёт от master → dev-коммиты не видны на GitHub". | (infrastructure, без deploy) |
| **Sprint 4 RAG revert** | `25fe53e` | **Sprint 4 RAG MVP (`eb4d9e1`) revert'нут** в конце сессии. Обоснование "deploy infra sync" больше неактуально (такой проблемы не существовало — deploy едет tar'ом). Revert revert = один коммит когда RAG будет готов к прод ingestion. | `20260907T081346Z-5c95974` |
...
- **Full pytest 1562 passed** (+32 к baseline 1530: +12 Sprint 3.39, +7 Sprint 4.2, +4 Sprint 4.1, +2 Sprint 4.5+, +3 Sprint 3.38, +4 quality batch), 30 skipped, 1 xfailed (legacy xfail), 5 warnings parallel (Pydantic × 4 workers + 1 pre-existing RuntimeWarning).
- **pytest-xdist -n 4:** 168s avg (×3.40 speedup). Flake-guard 4/4 pass.
- **HEAD `27d6f5e`** на remote `main` (единственная ветка-истина после реорганизации 2026-09-07: master ff-merged в main + design-audit-2026-08-20-fixes ff-merged; удалены 5 локальных и 3 remote feature-ветки — все коммиты сохранены в истории main). 41+ commits сессии (2026-09-06 → 2026-09-07).
- **Production:** latest deploy `20260907T081346Z-5c95974` (Sprint 4 RAG revert закрыл сессию). /health=200, smoke.sh=9/9 green. Раньше в этой сессии деплоилось ещё 3 раза: `070453Z-5c95974` (Sprint 4.1 + 4.2), `073517Z-5c95974` (Sprint 4.5+ + quality batch), `081346Z-5c95974` (revert RAG). Все green.
- **Sprint 4.4 (smoke user):** dedicated `smoke-test@431a.ru` (id=82) создан в прод-БД. pg_dump снапшот `db-pre-smoke-user-20260907-064638.sql.gz` (4.2 МБ) перед destructive INSERT. Credentials в `/opt/ai-tutor/deploy/release/.smoke.env` (chmod 600, НЕ в git).
- **Whitelist пароли:** НЕ ТРОНУТЫ (Sprint 4.4 = dedicated user, не перезапись существующих).
- **Frontend single-source refactor:** ~39 строк клиентской логики убрано (25 в parents/page.tsx, 14 в parent/dashboard/[studentId]/page.tsx).
